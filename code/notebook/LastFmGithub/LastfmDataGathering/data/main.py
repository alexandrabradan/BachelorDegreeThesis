...
state_variable = ""  # state variable to memorize the deleted user after restart of log


def check_if_file_is_corrupted(last_user_crawled):
    """
        Function which checks if the last user crawled is not corrupted after a suddently crash or interruption
        occured while we collected its data. We check if:
        1. the files and "data/users_info/" + last_user_crawled + "_info.json.gz"
           "data/users_listenings/" + last_user_crawled + "_listenings.json.gz" exists 
        2. the files aren't corrupted
        in order to dismiss and redo its collecting data or hold it
    """
...
def get_index_to_start_crawling(user_names, users_log_file_path):
    """
        Function which returns the index from which to start crawling the users present
        in the "data/users.csv" file. This function is called:
        1. at the beggining of the crawling process
        2. every time a crash or interruption are detech and the crawling process is restart
    """
...
def process_user(start_index, users, lfc, charts_type=("artist", "album", "track"), start_date=None, end_date=None):
    """
        Function which collects the network and personal data of the usesrs of our study
    """
...
def get_additional_infos(users, lfc):
    """
        Function which collects the listenings made by the users of our DataSets, during our period of study
    """
...
if __name__ == "__main__":
    ...
    # yo have to ask for a Last.fm API , in order to use the Last.fm's APIs
    lastfm_api = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    crawling_date_start = date(2017, 7, 15)  # starting date of out study
    crawling_date_end = date(2019, 7, 16)    # endign date of our study
    start_date_in_unix = time.mktime(crawling_date_start.timetuple())  # convert start date in unixtime
    end_date_in_unix = time.mktime(crawling_date_end.timetuple())  # convert end date in unixtime
    ...
    # dg.check_if_directory_exist(e "data/users_listenings/")
