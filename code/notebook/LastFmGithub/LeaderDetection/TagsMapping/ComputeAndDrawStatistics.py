...
def distribuition_tags_per_artists():
    ...
    # draw hystogram
    plt.bar(x, y, align='center', color="#ff737d", log=True, ec="white")  # A bar chart
    plt.xlabel('Tags (log10)')
    plt.ylabel('# Artists (log10)')
...
def distribuition_tags_per_artists_with_leaders():
    ...
    # draw hystogram
    plt.bar(x, y, align='center', color="#ff737d", log=True)  # A bar chart
    plt.xlabel('Tags')
    plt.ylabel('# Artists')
    plt.xticks(rotation=45)  # rotate x' labels in order to prevent crowdiness
...
def distribuition_artists_per_leaders():
    ...
    plt.xlabel("# Artists" + " (log10)")
    plt.ylabel("# Leaders" + " (log10)")
    plt.legend()
    plt.show()
...
def correlation_between_three_dimension_of_social_prominance():
    ...
    # Import Dataset
    df = pd.read_csv("final_leaders_DIRECTED2.csv", delimiter="\t", usecols=["width", "depth", "strength", "in_degree",
                                            "out_degree", "clustering", "closenness", "betweenness", "pagerank"])

    # Plot
    plt.figure()
    ax = sns.heatmap(df.corr(), xticklabels=df.corr().columns, yticklabels=df.corr().columns, cmap='RdYlGn_r', center=0,
                     annot=True)
