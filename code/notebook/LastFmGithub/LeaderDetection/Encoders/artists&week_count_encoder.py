...
def get_week_encoding(week):
    ...
    # Compile a regular expression pattern into a regular expression object, which can be used
    # for matching using its match(), search() and other methods
    b = bytes(str(week), 'utf-8')  # convert week to bytes
    my_regex = re.compile(b)
            ...
            if filename == "weeks_map1":
                ...
                with z.open(filename) as f:
                    for line in f:
                        ...
                        if match is not None:
                            # retrive week's encoding
                            week_encoding = line.split(b)
                            # get encoding
                            w_e = week_encoding[1].decode("utf-8")  # convert bytes to string
                            w_e2 = re.sub(r"[\n\t\s]*", "", w_e)
                            f.close()
                            z.close()
...
# add_file_to_zip_directory("artists_map1", "leader_detection.zip")
