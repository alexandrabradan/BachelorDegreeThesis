...
class Diffusion_old(object):
    ...
    def build_action_subgraph(self, action, delta):
        ...
        for e in isg.edges():  # action's subgraph pair edge list
            ...
            # difference between artist's first week of listening (encoded) made by the seed user and his following
            ...
            if diff != 0 and diff <= delta:
                # if the following user that the seed follows has listen first to the artist, I keep the edge
