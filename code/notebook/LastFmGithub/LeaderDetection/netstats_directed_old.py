        ...
        for filename in z.namelist():
            if filename == "network_filtered_int1.edgelist":
                with z.open(filename, 'r') as f:
                    ...
                    f.close()"""
    f = open("filtered_edge_list.csv")
    for line in f:
        line_array = line.split("\t")
        if line_array[0] == "Source":
            continue
        G.add_edge(int(line_array[0]), int(line_array[1].replace("\n", "")))
    f.close()
...
def draw_in_degree(x, x_label, y_label, global_bin_edges):
    ...
    x = [1 if elem == 0 else elem for elem in x]

    n, bin_edges, patches = ax1.hist(np.log10(x), log=True, color='#ff737d', ec="white")
    ...
    ax2.scatter(bin_centres, n, marker='o', color='#ff737d')
    ax2.set_yscale('log')
    ...
    popt, pcov = curve_fit(powlaw, bin_centres, y_pruned, p0=p0)
    print("popt = " + str(popt))
    print("pcov = " + str(pcov))
    ...
    # ax3.scatter(bin_centres, np.log(y_pruned), label='data', color='#ff737d')
    ax3.scatter(bin_centres, y_pruned, label='data', color='#ff737d')
    #ax3.set_yscale('log')
...
def draw_in_degree(x, x_label, y_label, global_bin_edges):
    ...
    # SET CUSTORM TICK FORMATTING (in my case log10)
    plt.gca().yaxis.set_major_formatter(FuncFormatter(lambda x, y: '{}'.format(float(math.ceil(math.log(x, 10))))))
    plt.gca().xaxis.set_major_formatter(FuncFormatter(lambda x, y: '{}'.format(float(math.ceil(math.log(x, 10))))))
    ...
    if len(global_bin_edges) != 0:
        n, bin_edges, patches = ax1.hist(x, log=True, bins=global_bin_edges, color='#ff737d', width=binwidth / 2)
        print("num_bins = " + str(len(n)))
    else:
        n, bin_edges, patches = ax1.hist(x, log=True, bins='doane', color='#ff737d', width=binwidth / 2)
        print("num_bins = " + str(len(n)))
...
def degree_study(G, start, final_leaders):
    ...
    degree_study_function_of_support(out_degree, final_leaders, "netstats_directed_old/directed_graph_out_degree", "Out-Degree", "# Nodes")
