...
class EarlyAdoptersThreshold(object):
    ...
    def load_splitted_db_data(self, main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge,
        ...
        # retrieve training set's adoptions
        self.insert_adoptions_into_training_set(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge)
        ...
        self.insert_adoptions_into_test_set(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge,
                                            test_set_start_edge, test_set_end_edge)
...
if __name__ == "__main__":
    ...
    for i in range(0, len(training_set_edges)):
        ...
        i_incr = i + 1
        database_name = "lastfm_AT" + str(i_incr) + ".db"

        ground_truth_file = "data/TRS" + str(i_incr) + "_at_ground_truth_file5"
        ...
        slots = 15  # 4 months of observation get best predictive results
        ...
        ground_truth_file = "data/ts" + str(i_incr) + "_at_ground_truth_file5"
        ...
        cur = conn.cursor()
        cur.execute("""SELECT * from stats_hit where min_redundancy='%d' and min_coverage='%d';""" % (hit_beta, hit_alpha))
        res = cur.fetchall()
        print(res)
        ...
        cur = conn.cursor()
        cur.execute("""SELECT * from stats_flop where min_redundancy='%d' and min_coverage='%d';""" % (flop_beta, flop_alpha))
        res = cur.fetchall()
        print(res)
