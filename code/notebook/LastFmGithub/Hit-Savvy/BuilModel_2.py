...
def insert_adoptions_into_training_set(main_db_name, training_set_db_name, training_set_start_edge,
...
def insert_adoptions_into_test_set(main_db_name, training_set_db_name, training_set_start_edge,
...
def load_splitted_db_data(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge,
                                                                        test_set_start_edge, test_set_end_edge):
    """
    ...
    # retrieve training set's adoptions
    insert_adoptions_into_training_set(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge)

    # build test set table (inside training set db)
    __build_table_for_test_set(training_set_db_name)
    # retrieve test set's adoptions
    insert_adoptions_into_test_set(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge,
                                        test_set_start_edge, test_set_end_edge)
...
def compute_adoption_trand_for_training_sets(num_training_set, period_of_continuity, training_set_start_edge, training_set_end_edge):
    f1 = "continous_at_in_TRS" + str(num_training_set) + ".json"
    f2 = "not_continous_at_in_TRS" + str(num_training_set) + ".json"

    conn = sqlite3.connect("lastfm.db")
    curr = conn.cursor()
    curr.execute("""SELECT distinct slot from adoptions where (slot >= '%d' and slot <= '%d')
                                order by slot asc""" % (training_set_start_edge, training_set_end_edge))
    weeks = curr.fetchall()
    tmp_dict = {}
    tmp_dict_not_cont = {}
    ...
    for g in artists:
        ...
        print("period_of_continuity TRS = " + str(period_of_continuity))
        curr = conn.cursor()
        curr.execute("""SELECT distinct adopter, slot from adoptions where (slot >= '%d' and slot <= '%d' and good='%s')
                                    order by slot asc""" % (training_set_start_edge, training_set_end_edge, str(g)))
        adopters = curr.fetchall()
        print(adopters)
...
def compute_adoption_trand_for_test_sets(num_training_set, period_of_continuity, test_set_starting_edge,
                                                                                                test_set_end_edge):
    f1 = "continous_at_in_ts" + str(num_training_set) + ".json"
    f2 = "not_continous_at_in_ts" + str(num_training_set) + ".json"

    training_db = "lastfm_AT" + str(num_training_set) + ".db"
    conn = sqlite3.connect(training_db)
    curr = conn.cursor()
    curr.execute("""SELECT distinct slot from adoptions_test order by slot asc""")
    weeks = curr.fetchall()
    tmp_dict = {}
    tmp_dict_not_cont = {}

    curr = conn.cursor()
    curr.execute("""SELECT distinct good from adoptions_test order by slot asc""")
    res = curr.fetchall()
    artists = []
    for g in res:
        g = int(g[0])
    ...
    for g in artists:
        ...
        print("period_of_continuity TRS = " + str(period_of_continuity))
        curr = conn.cursor()
        curr.execute("""SELECT distinct adopter, slot from adoptions_test where (good='%s')
                                    order by slot asc""" % (str(g)))
        adopters = curr.fetchall()
        print(adopters)
...
if __name__ == "__main__":
    ...
    training_set_edges = [(26, 41), (26, 57), (73, 61)]
    test_set_edges = [(42, 103), (58, 103), (74, 103)]
    ...
    for i in range(0, len(training_set_edges)):
        ...
        i_incr = i + 1
        training_set_db_name = "lastfm_AT" + str(i_incr) + ".db"

        # load current training set and test set db
        load_splitted_db_data(main_db_name, training_set_db_name, training_set_start_edge, training_set_end_edge, test_set_start_edge, test_set_end_edge)

        compute_adoption_trand_for_training_sets(i_incr, training_set_end_edge - training_set_start_edge,
                                                 training_set_start_edge, training_set_end_edge)
        ...
        evaluate_clustering_results(ts_not_continous_file, ts_clustering_res_file, ts_ground_truth_file)