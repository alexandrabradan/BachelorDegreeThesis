...
def update_files(start_index):
    data = pd.read_csv("seed_users_attributes.csv", delimiter="\t", usecols=["Source", "Target"])
    # print(data)

    new_file = open("seed_users_info.csv", "a", encoding="utf-8")
    id = "Id"
    label = "Label"
    continent = "Continent"
    country = "Country"
    main_music_genre = "Main_genre"
    new_file.write(f"{id}\t{label}\t{continent}\t{country}\t{main_music_genre}\n")
    ...
    for row in data.itertuples():
        ...
        node = int(row.adopter)
        country = row.country
        country_encoding = get_user_country(node)
        if country == -1:
            print("country of user = " + str(node))
            sys.exit(-1)

        countinent_encoding = get_user_continent(country)
        if countinent_encoding == -1:
            print("continet of user = " + str(node))
            sys.exit(-1)

        main_music_genre = get_user_main_genre(node)

        new_file.write(f"{node}\t{countinent_encoding}\t{country_encoding}\t{main_music_genre}\n")
        new_file.flush()
